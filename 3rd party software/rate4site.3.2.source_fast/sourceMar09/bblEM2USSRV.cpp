// 	$Id: bblEM2USSRV.cpp 920 2006-09-21 09:26:12Z ninio $	
...
MDOUBLE bblEM2USSRV::compute_bblEM(
	...
	for (int i=0; i < maxIterations; ++i) {
		...
		LOG(5,<<"current best  L= "<<currL<<" = "<< likelihoodComputation2USSRV::getTreeLikelihoodAllPosAlphTheSame(_et,_sc,_baseSc,_model)<<endl<<endl); 
	

		if (currL < oldL + epsilon) { // need to break
			if (currL<oldL) {
				cout<<"******** PROBLEMS IN BBL USSRV*********"<<endl;
				LOG(5,<<"The like in if = "<<likelihoodComputation2USSRV::getTreeLikelihoodAllPosAlphTheSame(_et,_sc,_baseSc,_model)<<endl);
				_et = oldT;
				return oldL; // keep the old tree, and old likelihood
			} else {
                //update the tree and likelihood and return
				LOG(5,<<"The like in else  = "<<likelihoodComputation2USSRV::getTreeLikelihoodAllPosAlphTheSame(_et,_sc,_baseSc,_model)<<endl);
				return currL;
			}
		}
		oldT = _et;
