...
# perl cn_2202a.pl -PDB 1.pdb -CHAIN A -Out_Dir /bioseq/Gershon/ConSurf_Exec/test -m
...
GetOptions(
	   ...
	   # Building MSA mode 
	    ...
	    "MaxHomol:s"=>\$FORM{MAX_NUM_HOMOL},	# <Max Number of Homologs to use for ConSurf Calculation> (deafult: 50)
...
print "Submuission Time: ".$VARS{submission_time};
...
$VARS{gradesPE} = "$FORM{pdb_ID}_$FORM{chain}_consurf.grades";
...
print "\ngot in mode : no msa - with PDB or without PDB MODE\n";
...
if ($VARS{running_mode} eq "_mode_pdb_no_msa" or $VARS{running_mode} eq "_mode_no_pdb_no_msa"){
    ...
    print "\ngot in mode : no msa - with PDB or without PDB MODE-22222\n";

    if ($VARS{running_mode} eq "_mode_pdb_no_msa" and (defined($VARS{SEQRES_seq}) and length($VARS{SEQRES_seq}) > 0)){
	    print "\ngot in mode : no msa - with PDB or without PDB MODE-33333\n";
...
#---------------------------------------------
    use Data::Dumper; 
    my $ref_r4s2pdb = shift;
#    print Dumper (@gradesPE_Output);
#    print Dumper ($ref_r4s2pdb);
#    print Dumper (%residue_freq);
#    print Dumper (@no_isd_residue_color);
#    print Dumper (@isd_residue_color);
#    print Dumper ("$VARS{working_dir}/$VARS{gradesPE}");
#    die();
