...
sub create_consurf_pipe_new{
	...
	if (($title_line eq "") or (!defined $title_line)){
...
sub create_atom_position_file{
	...
	if (!-e $atom_position_filename or -z $atom_position_filename){
		$ref_to_return_hash->{ERROR} = "cp_rasmol_gradesPE_and_pipe::create_atom_position_file : the file $atom_position_filename was not created or of size 0";
		return;
	}
...
# input: MSA - CLUSTALW format only!!
	...
	&print_chimera_scf_and_histogram($dir_path.$chimera_scf, $dir_path.$headers, \%aln_position_grade, $position_in_msa);
	&print_chimera_scf_and_histogram($dir_path.$chimera_scf_isd,$dir_path.$headers_isd, \%aln_position_grade_isd, $position_in_msa) if ($isd eq "yes");
	return ("OK",$isd);	
}
#************************************************************************************************
...
<body>
	<h3>Create a high resolution Chimera figure</h3>
EndOfHTML
    ...
    <ol>
	    <li>(a) <a href = "$isd_chimerax_script" type="application/x-chimerax">Click here</a> for PDB showing Insufficient Data<br />
	    (b) <a href = "$chimerax_script" type="application/x-chimerax">Click here</a> for PDB hiding Insufficient Data</li>
    </ol>
