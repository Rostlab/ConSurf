// $Id: posteriorDistance.cpp 759 2006-07-03 16:28:17Z ninio $
...
public:
	...
	for (int pos=0; pos < _s1.seqLen(); ++pos){
	    ...
	    sumL += convert(posLikelihood_d/posLikelihood)*(_weights ? (*_weights)[pos]:1.0);
	}
	return -sumL;
    };
};
...
const MDOUBLE posteriorDistance::giveDistance(const sequence& s1,
					      const sequence& s2,
					      const Vdouble  * weights,
					      MDOUBLE* score) const { /*DEBUG*/ LOG(9,<<"posteriorDistance::giveDistance - start"<<endl); LOGDO(9,printTime(myLog::LogFile())); /*DEBUG*/
