...
#----------------------------------    
    ...
    my $cd_hit = shift;   # cd-hit program
...
#----------------------------------    
    ...
    unless (open OUT_REJECT, ">".$working_dir.$rejected_seqs){
        {return ("sys", "prepareMSA::choose_homologoues_from_blast : can't open file $rejected_seqs for writing\n");}
    }    
	    ...
	    $s_name= $hit->name();
            ...
            while( my $hsp = $hit->next_hsp ) { #hsp is the next available High Scoring Pair, Bio::Search::HSP::HSPI object or null if finished
                ...
                if (exists $sequences{$s_name}){
                    $seq_name_exists = "yes";
                    $seq_details = $sequences{$s_name};
                    $ans = &check_if_seq_valid_with_min_id($redundancyRate, $min_length, $min_id_percent, $s_ident, $AAseq, $s_name);
                    if ($ans eq "yes"){
                        {$ans = &check_if_no_overlap($frag_overlap, $seq_details, $s_beg, $s_end);}
                    }                    
                }
...
#   HOMOLOUG SPECIFIC INPUT
    ...
    elsif ($ident_percent < $min_id){ $ans = "identity percent $ident_percent is too low (below $min_id)";}
