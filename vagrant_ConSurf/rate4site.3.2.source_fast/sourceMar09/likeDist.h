// $Id: likeDist.h 935 2006-10-17 08:38:01Z ninio $
...
public:
	...
	:  _sp(sp),_nonConstSpPtr(NULL),_toll(toll),_maxPairwiseDistance(maxPairwiseDistance),_jcDist(_sp.alphabetSize()) {}
  ...
  virtual likeDist* clone() const {return new likeDist(*this);}
    ...
    explicit likeDist(stochasticProcess& sp,
		      const MDOUBLE toll =0.0001,
		      const MDOUBLE maxPairwiseDistance = 5.0)
	:  _sp(sp),_nonConstSpPtr(&sp),_toll(toll),_maxPairwiseDistance(maxPairwiseDistance),_jcDist(_sp.alphabetSize()) {}
