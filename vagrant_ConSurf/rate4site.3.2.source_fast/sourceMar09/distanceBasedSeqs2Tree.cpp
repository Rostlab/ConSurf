// $Id: distanceBasedSeqs2Tree.cpp 923 2006-09-25 16:21:38Z eyalprivman $
...
void iterativeDistanceSeqs2Tree::seqs2TreeOneIterationInternal(const sequenceContainer &sc, const bool sideInfoSet) {
	...
	if (!sideInfoSet) { // Then use homogeneous rates
		...
		stochasticProcess uniDistSp (&distribution, _spPtr->getPijAccelerator());
...
MDOUBLE commonAlphaDistanceSeqs2Tree::optimizeSideInfo(const sequenceContainer &sc, tree &et)
{
	bestAlphaAndBBL optimizer(et, sc, *_spPtr, _weights, _alpha, 5.0,
							  _epsilonLikelihoodImprovement4BBL/*0.01*/, _epsilonLikelihoodImprovement4alphaOptimiz); //, _args_info.ADVNumOfBBLIterInBBLPlusAlpha_arg);
...
{
	bblEM optimizer(et, sc, *_spPtr, _weights, 50, _epsilonLikelihoodImprovement);
 ...
 ********************************/
tree posteriorDistanceSeqs2Tree::seqs2TreeIterative(const sequenceContainer &sc, const VVdoubleRep &initPosterior, const Vdouble *weights, const tree* constraintTreePtr) {
